...
//get combining page
router.route("/laboratory/combining/lost")
    .get(
        async (req, res) => {
            if(req.session.user == null) {
                ...
                let part14 = await getUserAssetByPart(14, req.session.user.address)
                res.render(
                    "./laboratory/combining/lostGeneration",
                    {
                        title: "Combining",
                        haveUser: true,
...
router.route("/laboratory/combining/lost/canmint")
    .post(
        async (req, res) => {
            let chance = await getChance(req.session.user.address)
            let substrate = await getSubstrate(
            ...
            res.render(
                "./laboratory/combining/lostGeneration",
                {
                    ...
                    part14: await getUserAssetByPart(14, req.session.user.address)
                }
            ):
            res.redirect("/laboratory/combining/lost")
        }
    )
