...
//get combining page
router.route("/laboratory/combining/zero")
    .get(
        async (req, res) => {
            if(req.session.user == null) {
                ...
                let part6 = await getUserAssetByPart(6, req.session.user.address)
                res.render(
                    "./laboratory/combining/zeroGeneration",
                    {
                        title: "Combining",
                        haveUser: true,
...
router.route("/laboratory/combining/zero/canmint")
    .post(
        async (req, res) => {
            let chance = await getChance(req.session.user.address)
            let substrate = await getSubstrate(
            ...
            res.render(
                "./laboratory/combining/zeroGeneration",
                {
                    ...
                    part6: await getUserAssetByPart(6, req.session.user.address)
                }
            ):
            res.redirect("/laboratory/combining/zero")
        }
    )
