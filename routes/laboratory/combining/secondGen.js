...
//get combining page
router.route("/laboratory/combining/second")
    .get(
        async (req, res) => {
            if(req.session.user == null) {
                ...
                res.render(
                    "./laboratory/combining/secondGeneration",
                    {
                        title: "Combining",
                        haveUser: true,
...
router.route("/laboratory/combining/second/canmint")
    .post(
        async (req, res) => {
            let chance = await getChance(req.session.user.address)
            let substrate = await getSubstrate(
            ...
            res.redirect("/laboratory/combining/second")
        }
    )
