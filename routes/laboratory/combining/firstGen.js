...
//get combining page
router.route("/laboratory/combining/first")
    .get(
        async (req, res) => {
            if(req.session.user == null) {
                ...
                res.render(
                    "./laboratory/combining/firstGeneration",
                    {
                        title: "Combining",
                        haveUser: true,
...
router.route("/laboratory/combining/first/canmint")
    .post(
        async (req, res) => {
            let chance = await getChance(req.session.user.address)
            let substrate = await getSubstrate(
            ...
            res.redirect("/laboratory/combining/first")
        }
    )
