...
async function toggleButton() {
    ...
    .then((res)=>{
        if (res.status == true) {
            fetch("/store/things/buy", {
                method: "POST",
                headers: {
                  'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    note: {id: 0, amount: inputs[0].value},
                    booster: {id: 1, amount: inputs[1].value},
                    snare: {id: 2, amount: inputs[2].value},
                    trap: {id: 3, amount: inputs[3].value},
                    bait: {id: 4, amount: inputs[4].value}
                })
            })
            alert("You just bougth things!")
        } else {
            return;
        }
    })
